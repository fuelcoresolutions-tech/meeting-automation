...
### Retry System
The system includes a durable retry mechanism that handles:
- API credit exhaustion (automatic retry when credits restored)
- Network failures and rate limits
- Extended payment gaps (weeks+)

**Key Environment Variables:**
- `ENABLE_DURABLE_RETRY_WORKER=true` - Background retry worker
- `RETRY_BACKOFF_SECONDS=60,300,900,3600,21600,43200,86400` - Retry schedule
- `STALE_REQUEUE_HOURS=12` - Auto-requeue stuck meetings

**Manual Controls:**
- `POST /worker/retry-pending-now` - Force immediate retry
- `POST /worker/retry-meeting/{id}` - Retry specific meeting
- `POST /worker/force-rerun/{id}` - Force rerun completed meeting

### Emergency Recovery
For extended outages, see `ops/backfill-procedure.md` for backfill instructions.

### Monitoring
Check Meeting Register for:
- `processingStatus` - Current state
- `retryCount` - Number of retry attempts
- `lastErrorMessage` - Failure details
