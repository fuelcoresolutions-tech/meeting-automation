...
# Long meeting threshold (tokens) — set high to leverage 1M context window directly
LONG_MEETING_THRESHOLD = int(os.getenv("LONG_MEETING_THRESHOLD_TOKENS", "900000"))

# Define tools for Claude API (function calling)
TOOLS = [
    {
        "name": "get_projects",
        "description": "Retrieve all existing projects from Notion including descriptions, keywords, and client info. Use this first to find relevant projects for linking tasks and notes. When cross-project topics are detected, create ONE meeting note and link it to ALL relevant projects via project_ids.",
        "input_schema": {
            "type": "object",
            "properties": {},
            "required": []
    ...
    },
    {
        ...
        "input_schema": {
            ...
            "properties": {
                ...
                "project_ids_secondary": {"type": "array", "items": {"type": "string"}, "description": "Deprecated alias field. Do not use. Use project_ids instead for multi-project linking."},
...
Use ONLY project_id/project_ids from the list above when creating notes and tasks.
Do NOT guess or use a fallback project if mapping is unclear.
If no project matches clearly, DO NOT create meeting note to prevent wrong assignments.
...
"""
    ...
    try:
        ...
        for iteration in range(max_iterations):
            ...
            # Process response
            # Handle Sonnet 4.6 stop reasons
            if response.stop_reason == "refusal":
                logger.warning("Claude refused to process this request — stopping agentic loop")
                break
            if response.stop_reason == "model_context_window_exceeded":
                logger.error("Context window exceeded — transcript too large even for 1M context window")
                break
