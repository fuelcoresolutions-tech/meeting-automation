...
**ONE TASK PER DISTINCT ITEM — NEVER MERGE:**
When a speaker mentions multiple distinct items in one statement, create a SEPARATE task for EACH one.
- "We need two AI subscriptions — one for the chat interface and one for the developer API" → TWO tasks
- "Get me the report and also update the website" → TWO tasks
- "John Mark should call Stabex and also follow up with the Mbale depot" → TWO tasks
This applies to any list, enumeration, or "and also" structure. Count the items explicitly and create that many tasks.
Never consolidate: "Set up AI tools (chat + developer)" is WRONG — it hides the second item.
...
- If zero projects match → DO NOT create meeting note; log error and skip to prevent wrong project assignments
