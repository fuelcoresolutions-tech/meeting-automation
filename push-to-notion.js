  ...
  for (const data of allFiles) {
    for (const issue of (data.eos_issues || [])) {
      ...
      try {
        ...
        // Only link when a project is explicitly resolved.
        // This avoids cross-project data mixing caused by implicit defaults.
        const resolvedIssueProjectId = resolveProjectId(issue.project_name || issue.project_id, ctx.projects);
        if (resolvedIssueProjectId) {
          issueData.projectIds = [resolvedIssueProjectId];
        } else {
          console.log(`  ⚠️ Skipping project link for issue "${issue.issue_title}" (no deterministic project match)`);
        }
...
async function pushTasks(allFiles, ctx) {
  ...
  for (const data of allFiles) {
    for (const task of (data.tasks || [])) {
      ...
      try {
        const projectId = resolveProjectId(task.project_name || task.project_id, ctx.projects);
        if (!projectId) {
          console.log(`  ⚠️ Skipping task "${task.name}" because project match is missing`);
          skipped++;
          continue;
        }
        const taskData = {
